...
void drawFrame(Board *board, Texture2D *spriteSheet, Rectangle *spriteRecs, DrawingPieceMouseHandler *drawingPieceMouseHandler, Sound *sounds, int showIndexes, LegalMovesContainer *curLegalMoves, TranspositionTable *tt, SearchRootResult *result, int *draggingPieceType, OpeningBook *book, char *notation, Font *myFont)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            if (showIndexes)
            {
                DrawTextEx(*myFont, text, (Vector2){i * 75, j * 75}, 40, 1, GREEN);
            }

            if (j == 7)
            {
                if (i == 6) {
                    DrawTextEx(*myFont, TextFormat("%c", "abcdefgh"[i]), (Vector2){i * 75, (j * 75) + 40}, 35, 1, BLACK);
                } else {
                    DrawTextEx(*myFont, TextFormat("%c", "abcdefgh"[i]), (Vector2){i * 75, (j * 75) + 45}, 35, 1, BLACK);
                }
            }

            if (i == 7)
            {
                DrawTextEx(*myFont, TextFormat("%c", "87654321"[j]), (Vector2){(i * 75) + 59, (j * 75)}, 40, 1, BLACK);
            }

            if (kToDraw != -1)
            {
                snprintf(text, sizeof(text), "%d", kToDraw);
                DrawTextEx(*myFont, text, (Vector2){i * 75, j * 75 + 50}, 40, 1, WHITE);
            }
        }
    }
    ...
    if (board->gameState == CHECKMATE)
    {
        ...
        DrawTextEx(*myFont, text, (Vector2){1 * 75, 3 * 75}, 90, 1, DARKGREEN);
    }
    else if (board->gameState == DRAW || board->gameState == DRAW_RETURN_0_IN_SEARCH)
    {
        char text[] = "DRAW!";
        DrawTextEx(*myFont, text, (Vector2){2.5 * 75, 3.5 * 75}, 90, 1, DARKGREEN);
    }
